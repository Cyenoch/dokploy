...
interface CDNProviderConfig extends CDNProvider {
	ipRanges?: readonly string[];
	ipAddresses?: ReadonlySet<string>;
	hostnameSuffixes?: readonly string[];
}

type CDNProviderLookup =
	| {
			type: "ip";
			ip: string;
	  }
...
const matchesHostnameSuffix = (hostname: string, suffix: string) => {
...
const CDN_PROVIDERS: CDNProviderConfig[] = [
	{
		name: "cloudflare",
		displayName: "Cloudflare",
		ipRanges: CLOUDFLARE_IP_RANGES,
		warningMessage:
...
const matchesProvider = (
	...
	lookup: CDNProviderLookup,
) => {
	if (lookup.type === "ip") {
		return (
			provider.ipAddresses?.has(lookup.ip) ||
			provider.ipRanges?.some((range) => isIPInCIDR(lookup.ip, range)) ||
			false
		);
	}

	return (
		provider.hostnameSuffixes?.some((suffix) =>
			matchesHostnameSuffix(lookup.hostname, suffix),
		) || false
	);
};
...
export const detectCDNProvider = (
	...
	return (
		CDN_PROVIDERS.find((provider) => matchesProvider(provider, lookup)) || null
	);
};
